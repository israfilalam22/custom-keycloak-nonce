...
public class DefaultSecurityHeadersProvider implements SecurityHeadersProvider {
    ...
    public void addHeaders(ContainerRequestContext requestContext, ContainerResponseContext responseContext) {
        ...
        if (responseType == null && !isEmptyMediaTypeAllowed(requestContext, responseContext)) {
            LOGGER.errorv(
                    "MediaType not set on path {0}, with response status {1}",
                    session.getContext().getUri().getRequestUri().getPath(),
                    responseContext.getStatus()
            );
            throw new InternalServerErrorException();
        }

        boolean rest = isRest(requestType, responseType);
        boolean html = isHtml(requestType, responseType);
    ...
    private void addHtmlHeaders(MultivaluedMap<String, Object> headers) {
        ...
        if (cspVal != null) {
            ContentSecurityPolicyBuilder csp =
                    ContentSecurityPolicyBuilder.create(cspVal.toString());

            String nonce = KeycloakSecurityHeadersFilter.getNonce(session);

            if (nonce != null) {
                csp.addScriptSrc(ContentSecurityPolicyBuilder.DIRECTIVE_VALUE_SELF);
                csp.addScriptSrc("'nonce-" + nonce + "'");

                csp.addStyleSrc(ContentSecurityPolicyBuilder.DIRECTIVE_VALUE_SELF);
                csp.addStyleSrc("'nonce-" + nonce + "'");
            }

            /*
             * Security header options are optional and are initialized
            ...
            headers.putSingle(
                    CONTENT_SECURITY_POLICY.getHeaderName(),
                    csp.build()
            );
        }
    }
